...
public abstract class Monster {
	...
	public Attaque choixAttaqueBOT(Monster m) {
		...
		for (Attaque i : listAttaque) {
			if (Context.getInstance().getDAOAtk().ratioEfficacite(i.getType(),m.getType())==2) {
				r = new Random();
				if(r.nextInt(4)==4) {
					a=i;
				}
	...
	public static ArrayList<Attaque> creationAttaque(Integer[] poolEntier) {
		...
		return Context.getInstance().getDAOAtk().selectPoolId(idsForQuery);
	...
	public ArrayList<Attaque> poolAttaque(ArrayList<Integer> ids) {
		...
		this.listAttaque = Context.getInstance().getDAOAtk().selectPoolId(ids);
		return listAttaque;
	}
	...
	public void combat(Monster m) throws PVException {
		...
		if (r.nextInt(100)>a.getPrecision()) {
			...
			double type = Context.getInstance().getDAOAtk().ratioEfficacite(a.getType(),m.getType());
			if (type == 2) {
